function [hObject,handles]=bci_fESI_RunBCI(hObject,handles)
...
%                 output(3)=-100;
                ...
                if isequal(tasktype,4) % Noise Run
                    Dat.eeg=single(data);
                    Dat.begsample=begsample;
                    Dat.endsample=endsample;
                    Dat.event.bci2event=[];
                    if ~isequal(size(BCI2Event),[0 0])
                        if ~exist('PrevBCI2Event','var') ||...
                            ...
                            if strcmp(BCI2Event(end).type,'TargetCode') &&...
                                ...
                                Dat.event.bci2event=BCI2Event(end);
                            end
                        end
                        PrevBCI2Event=BCI2Event(end);
                    end
                    Dat.event.event2bci=Event2BCI;
                    if isequal(size(num2str(win),2),1)
                        WIN=strcat('00',num2str(win));
                    elseif isequal(size(num2str(win),2),2)
                        WIN=strcat('0',num2str(win));
                    else
                        WIN=num2str(win);
                    end
                    SaveFile=strcat(DatSaveFile,WIN,'.mat');
                    save(SaveFile,'Dat','-v7.3');
                else
                    Dat(win).eeg=(data);
                    Dat(win).psd.sensor=(ESensor);
                    if isequal(domain,3)
                        Dat(win).psd.source=(J);
                    end
                    Dat(win).begsample=begsample;
                    Dat(win).endsample=endsample;
                    Dat(win).controlsig.orig=control;
                    Dat(win).controlsig.norm=controlnorm(:,win);
                    Dat(win).controlsig.filt=controlfilt;
                    Dat(win).feedback=feedback(win);
                    Dat(win).event.bci2event=[];
                    
                    if exist('targetXpos','var') && exist('targetYpos','var')
                        Dat(win).targetpos=[targetXpos targetYpos];
                    end
                    
                    if exist('cursorXpos','var') && exist('cursorYpos','var')
                        Dat(win).cursorpos=[cursorXpos cursorYpos];
                    end
                    ...
                    Dat(win).event.event2bci=Event2BCI;
                end
