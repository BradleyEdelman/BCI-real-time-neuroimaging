...
tmp.txt <- read.table("M:/Dropbox/He_Lab/Papers/BCI_CP/Sci_Robo_20180421/R2/Materials/Temporal/Stats/R2_2D_Dynamics_2_Segments.txt", header=TRUE)
...
for (i in 1:length(vars)){
  ...
  myData.mean <- aggregate(myData$tmp,
                           by = list(myData$subjid, myData$cond, myData$time, myData$pair, myData$pair2, myData$pair3), FUN = 'mean')
  # head(myData.mean)
  
  colnames(myData.mean) <- c("subjid","cond","time","pair","pair2","pair3","var")
  ...
  # Tukey HSD post Hoc
  var.aov <- with(aov(var ~ pair3),data=myData.mean)
  #print(TukeyHSD(var.aov))
  tmp<-TukeyHSD(var.aov)
  tmp2<-data.frame(tmp$pair3)
  tmp3<-tmp2["p.adj"]
  print(tmp3)
...
pairwise.t.test(myData$UD,myData$pair,p.adj="bonferroni")
pairwise.t.test(myData$UD,myData$pair2,p.adj="bonferroni")